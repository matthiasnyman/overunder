...
const OVER = ['2', '4', '6', '8', '10'];
const UNDER = ['ACE', '3', '5', '7', '9'];
...
const gameStateReducer = (state = DEFAULT_GAME_STATE, action) => {
  switch(action.type) {
    ...
    case DECK_DRAW.FETCH_SUCCESS:
      ...
      if(
        (guess === 'over' && OVER.includes(value)) ||
        (guess === 'under' && UNDER.includes(value))
      ) {
        return { ...state, correctGuesses: correctGuesses+1 };
      }else{
        return { ...state, SET_GAME_STARTED: !SET_GAME_STARTED };
